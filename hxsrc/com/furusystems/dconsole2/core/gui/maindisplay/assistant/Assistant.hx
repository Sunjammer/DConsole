package com.furusystems.dconsole2.core.gui.maindisplay.assistant ;
	 ...
	 */
	 class Assistant extends Sprite implements IContainable implements  IThemeable {
		var _infoField:TextField;
		var _idle:Bool = true;
		var _snapshot:BitmapData;
		var _snapshotDisplay:Bitmap = new Bitmap();
		var _time:Float = 0;
		var _prevTimeUpdate:Float = 0;
		var _cornerHandle:CornerScaleHandle;
		...
		function onFrameUpdate(md:MessageData) {
			if (text == "")
				return;
			if (_infoField.maxScrollH > 0) {
				_time += flash.Lib.getTimer() - _prevTimeUpdate;
				_infoField.scrollH = Std.int((Math.sin(_time / 1000) + 1) / 2 * _infoField.maxScrollH);
				_prevTimeUpdate = flash.Lib.getTimer();
			} else {
				_time = _prevTimeUpdate = 0;
			}
		}
		
		function onClearRequest(md:MessageData) {
			clear();
		}
		
		function onMessageRequest(md:MessageData) {
			text = Std.string(md.data);
		}
		
		function onMouseDown(e:MouseEvent) {
			stage.addEventListener(MouseEvent.MOUSE_UP, onMouseUp);
			_snapshot = new BitmapData(Std.int(_infoField.width), Std.int(_infoField.height), false, 0);
...
return s;
		 ...
		 * @param	s
		 */
		public function setWeakText(s:String) {
			_infoField.text = s;
			_idle = true;
		}
		
		public function clear() {
			setWeakText(Strings.ASSISTANT_STRINGS.get(com.furusystems.dconsole2.core.strings.AssistantStringCollection.DEFAULT_ID));
			//text = "";
			_idle = true;
		}
...
function  get_minWidth():Float {
		...
		@:flash.property public var idle(get,never):Bool;
function  get_idle():Bool {
			return _idle;
		}
		
		@:flash.property public var cornerHandle(get,never):CornerScaleHandle;
function  get_cornerHandle():CornerScaleHandle {
