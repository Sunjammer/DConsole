package com.furusystems.dconsole2.core.input ;
	 ...
	 * @version 0.1
	 */
	 final class KeyboardShortcuts implements KeyboardList {
		static var INSTANCE:KeyboardShortcuts = null;
		var keyboardShortcuts:Vector<KeyboardShortcut> = new Vector();
		 ...
		 */
		@:flash.property public static var instance(get,never):KeyboardShortcuts;
static function  get_instance():KeyboardShortcuts {
			if (INSTANCE == null) {
		 ...
		 */
		public function add(keystroke:UInt, modifier:UInt, callback:ASFunction, override:Bool = false, replaceAll:Bool = true):Bool {
			var success = false;
		 ...
		 */
		public function remove(keystroke:UInt, modifier:UInt):Bool {
			var success= false;
			if (!isEmpty()) {
				if (has(keystroke, modifier)) {
					var i= 0;
					var l= keyboardShortcuts.length;while (i < l) {
...
i++;
		 ...
		 */
		function has(keystroke:UInt, modifier:UInt):Bool {
			var success= false;
			var i= 0, l= keyboardShortcuts.length;while (i < l) {
				if (inKeyboardShortcut(keystroke, modifier, keyboardShortcuts[i])) {
					success = true;
					break;
				}
i++;
		...
		public function onKeyDown(event:KeyboardEvent):Bool {
			...
			if (!isEmpty()) {
				...
				// Get the modifier
				var modifier= getModifierFromKeyboardEvent(event);
				/*
...
i++;
		 ...
		 */
		public function validateKeystrokeWithModifier(keystroke:UInt, modifier:UInt):Bool {
			var success= true;
			/*
		 ...
		 */
		function validateKeystroke(keystroke:UInt):Bool {
			var success= true;
			switch (keystroke) {
		 ...
		 */
		function validateModifier(modifier:UInt):Bool {
			var success= false;
			switch (modifier) {
				case KeyBindings.NONE
				   ...
				   | KeyBindings.CTRL_ALT
				   | KeyBindings.CTRL_SHIFT:
					success = true;
		 ...
		 */
		function isCombinedModifier(modifier:UInt):Bool {
			var success= false;
			switch (modifier) {
		 ...
		 */
		function isKeystrokeFN(modifier:UInt):Bool {
			var success= false;
			switch (modifier) {
				case KeyBindings.F1
				   | KeyBindings.F2
		 ...
		 */
		function addKeyboardShortcut(keystroke:UInt, modifier:UInt, callback:ASFunction) {
			keyboardShortcuts.push(new KeyboardShortcut(keystroke, modifier, callback));
		}
		 ...
		 * @param modifier	The modifier value.
		 */
		function removeKeyboardShortcut(keystroke:UInt, modifier:UInt) {
			var i= 0;
			var l= keyboardShortcuts.length;while (i < l) {
				if (inKeyboardShortcut(keystroke, modifier, keyboardShortcuts[i])) {
					break;
...
i++;
		 ...
		 */
		function inKeyboardShortcut(keystroke:UInt, modifier:UInt, keyboardShortcut:KeyboardShortcut):Bool {
		 ...
		 */
		function getModifierFromKeyboardEvent(event:KeyboardEvent):UInt {
			var modifier= KeyBindings.NONE;
			if (event.altKey) {
