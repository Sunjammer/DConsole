...
;
	 ...
	 */
	 class TransformTab extends PropertyTab
	{
		
		public function new(console:IConsole, target:IntrospectionScope) 
		{
			super("Transform", false);
			populate(console,target);
		}
		public function populate(console:IConsole, scope:IntrospectionScope) {
			if (!Std.is(scope.targetObject , DisplayObject)) throw new Error("Cannot populate transform tab with non displayobject scope");
